...
- **Nomad** 集群（≥ 2.0；已基于 `hashicorp/nomad:2.0.6` 验证）
...
里程碑 M0–M3c 已完成（工程骨架、连接层、总览 / 节点 / Job、指标流水线、Job 管理、部署观测）。Darwin / Windows / Linux 构建与打包已通过 Wails Taskfile 支持（`wails3 build` / `wails3 package`）。当前依赖 **Wails v3.0.0-beta.21**。
